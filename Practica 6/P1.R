...
getEstimator <- function(values){
	result <- maxlogL(values, dist = 'dbinom', start = c(size = 40, prob = 0.4)) # Size = 40 porque son 40 ensayos y prob inicial = 0.4 para ver si converge rápido a la proporción eficiente.
	#summary(result)
...
x <- c(25, 31, 17, 26, 12, 21, 18, 23)

estimators <- getEstimator(x)
size <- round(estimators[1])
prob <- estimators[2]

print(prob) # Imprimimos el estimador inicial de la probabilidad de éxito
